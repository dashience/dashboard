...
//        'is_not_null'
    ...
    QueryBuilder.prototype.createRuleInput = function (rule) {
        ...
        for (var i = 0; i < rule.operator.nb_inputs; i++) {
            ...
            if (i > 0)
                $valueContainer.append(this.settings.inputs_separator);
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    QueryBuilder.extend(/** @lends module:plugins.Invert.prototype */ {
        ...
        invert: function (node, options) {
            ...
            if (options.invert_rules === undefined)
                options.invert_rules = true;
            if (options.silent_fail === undefined)
                options.silent_fail = false;

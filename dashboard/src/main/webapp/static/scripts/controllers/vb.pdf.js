app.controller('PdfController', function ($stateParams, $http, $scope, $filter, $cookies) {
    ...
    $http.get("admin/ui/dbWidget/" + $stateParams.tabId + "/" + $stateParams.accountId).success(function (response) {
        ...
        var pdfUserWidgetItems = [];
        var pdfDefaultWidgetItems = [];
        pdfUserWidgetItems = response;
        pdfDefaultWidgetItems = response;
        $http.get("admin/settings/getSettings").success(function (response) {
            var defaultWidgetColors;
            if (response.defaultChartColor) {
                defaultWidgetColors = response.defaultChartColor.split(',');
            }
            pdfDefaultWidgetItems.forEach(function (value, key) {
                value.chartColors = defaultWidgetColors;
            });
            $http.get("admin/ui/getChartColorByUserId").success(function (response) {
                console.log(response);
                var widgetColors = null;
                if (response.optionValue) {
                    widgetColors = response.optionValue.split(',');
                }
                pdfUserWidgetItems.forEach(function (value, key) {
                    value.chartColors = widgetColors;
                });
                if (widgetColors == null) {
                    $scope.pdfWidgets = pdfUserWidgetItems;
                } else {
                    $scope.pdfWidgets = pdfDefaultWidgetItems;
                }
